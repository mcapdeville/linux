...
static ssize_t vs10xx_read(struct file *file, char __user *usrbuf, size_t lbuf, loff_t *ppos) {
	int id = (int)file->private_data;
	int status = 0;
...
static long vs10xx_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	...
	switch (iocnr) {
		...
		case _IOC_NR(VS10XX_CTL_GETSCIREG):
			if (copy_from_user(&scireg, usrbuf, iocsize))
				return -EFAULT;
			vs10xx_device_getscireg(id, &scireg);
			if (copy_to_user(usrbuf, &scireg, iocsize))
				return -EFAULT;
			break;
...
int vs10xx_create_device(int i) {
	int status;
	struct device *dev;

	/* init io path */
	if ((status=vs10xx_io_init(i)) == 0) {
...
MODULE_DESCRIPTION("VS10XX codec SPI device interface");
MODULE_LICENSE("GPL");
MODULE_ALIAS("spi:"VS10XX_SPI_CTRL);
MODULE_ALIAS("spi:"VS10XX_SPI_DATA);
