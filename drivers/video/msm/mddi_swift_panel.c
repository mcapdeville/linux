...
static void display_table(struct display_table *table, unsigned int count)
{
	...
	for(i = 0; i < count; i++) {
		...
		default:
			if (!in_interrupt())
				mddi_host_register_multiwrite(reg, table[i].val_list, table[i].count,FALSE,NULL,MDDI_HOST_PRIM);
			else
			{
				for (j=0;j<table[i].count;j++)
						mddi_host_register_write_int(reg,table[i].val_list[j],NULL,MDDI_HOST_PRIM);
			}
